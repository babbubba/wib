...
export class DashboardComponent implements OnInit {
  ...
  // Convert UTC datetime from API to local datetime-local format (YYYY-MM-DDTHH:mm)
  private utcToLocal(utcDatetimeString: string): string {
  ...
  private localToUtc(localDatetimeString: string): string {
  ...
  addNewLine() {
    ...
    this.http.post(`/api/receipts/${rec.id}/edit`, body).subscribe({
      next: () => {
        ...
        // Reload receipt from API to get updated data
        this.loadReceipt(rec.id);
      },
  ...
  saveEdits() {
    ...
    const ts = this.editDatetime(); if (ts) body.datetime = this.localToUtc(ts);
    this.http.post(`/api/receipts/${rec.id}/edit`, body).subscribe({
      next: () => {
        this.error.set('');
        // Reload receipt from API to get the updated datetime
        this.loadReceipt(rec.id);
      },
      error: e => this.error.set(this.apiErr(e, 'Salvataggio fallito'))
    });
  }
